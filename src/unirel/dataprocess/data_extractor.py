...
def unirel_extractor(
    ...
    for tail_pred, tail_label in zip(tail_preds, tail_labels):
        ...
        e2e_plain_state_dict["c"] += len(
            set(pred_plain_ee_list) & set(gold_plain_ee_list)
        )
    ...
    return (
        all_metirc_results["all-prec"],
        all_metirc_results["all-recall"],
        all_metirc_results["all-f1"],
    )
